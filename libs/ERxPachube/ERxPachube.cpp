...
ERxPachube::ERxPachube(const char* APIKey, unsigned int feedId) 
	: mAPIkey(), mFeedId(), mDatasteamLength(0)
{
	char strFeedIdBuffer[COSM_FEED_ID_MAX_LENGTH];
	itoa((signed long)feedId, strFeedIdBuffer, COSM_FEED_ID_MAX_LENGTH);
	setKeyAndFeed(APIKey, strFeedIdBuffer);
}

ERxPachube::ERxPachube(const char* APIKey, const char* feedId) 
	: mAPIkey(), mFeedId(), mDatasteamLength(0)
{
	setKeyAndFeed(APIKey, feedId);
}

void ERxPachube::setKeyAndFeed(const char* APIKey, const char* feedId)
{
	// Clear the buffer.
	mAPIkey[0] = '\0';
	mFeedId[0] = '\0';
	...
	if(APIKey)
	{
		const unsigned int strLen = strlen(APIKey);
		unsigned int length = COSM_API_KEY_MAX_LENGTH - 1;
		length = strLen > length ? length : strLen;
		strncpy(mAPIkey, APIKey, length);
		mAPIkey[length] = '\0';
	}

	if(feedId)
	{
		const unsigned int strLen = strlen(feedId);
		unsigned int length = COSM_FEED_ID_MAX_LENGTH - 1;
		length = strLen > length ? length : strLen;
		strncpy(mFeedId, feedId, length);
		mFeedId[length] = '\0';
	}
}
...
const char* ERxPachube::getAPIKey() const
{
	return mAPIkey;
}

const char* ERxPachube::getFeedId() const
{
	return mFeedId;
}
...
ERxPachubeDataOut::ERxPachubeDataOut(const char* APIKey, const char* feedId)
:ERxPachube(APIKey, feedId)
{
...
ERxPachubeDataIn::ERxPachubeDataIn(const char* APIKey, const char* feedId)
: ERxPachube(APIKey, feedId)
{
